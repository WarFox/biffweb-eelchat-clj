...
(defn assoc-result
  [{:keys [biff/base-url]} {:sub/keys [url last-modified etag] :as sub}]
...
(defn format-post
  [{:keys [title author published-date updated-date link contents]}]
...
(defn sub-tx
  [{:sub/keys [chan last-post-uri] :keys [xt/id ::result]}]
